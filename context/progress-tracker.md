...
- Wire the editor home sidebar and dialogs to the real project API/server data helper using the shared project actions hook on the `/editor` route.
...
- Feature 07: complete the requested server-side project helper and shared client action flow so the editor home sidebar and dialog manager can create, rename, and delete Liveblocks-aligned projects through the REST endpoints without mock-only project lists.
